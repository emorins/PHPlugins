...
namespace BoneModHarmony
{
    static class UIUtils
    {
        ...
        internal static void TweakGUI(ref float value, float range, string labeltext, string tooltip = "", string valuedecimals = "N3")
        {
            value = TweakGUI(value, range, labeltext, tooltip, valuedecimals);
        }
        internal static float TweakGUI(float value, float range, string labeltext, string tooltip = "", string valuedecimals = "N3")
        {
            GUILayout.BeginHorizontal();
            GUILayout.Label(new GUIContent(labeltext, tooltip), labelstyle);
            GUILayout.FlexibleSpace();
            if (GUILayout.Button(GUIStrings.Flip, buttonstyleNoStretch))
            {
                value = -value;
            }
            if (GUILayout.Button(GUIStrings.Reset, buttonstyleNoStretch))
            ...
            GUILayout.EndHorizontal();
            GUILayout.Label(value.ToString(valuedecimals), labelstyle2);
            value = GUILayout.HorizontalSlider(value, -range, range, sliderstyle, thumbstyle);
            return value;
        }
        ...
        static Color selected = new Color(0.98f, 0.65f, 0.14f);
        static Color selectedOnHover = new Color(0.8f, 0.5f, 0.1f);
